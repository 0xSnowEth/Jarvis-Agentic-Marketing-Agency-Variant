...
function resetClientIntakeForm() {
  ['c-name', 'c-context', 'c-phone', 'c-token', 'c-fb', 'c-ig'].forEach(id => {
  ...
  if (fileStatus) fileStatus.textContent = 'Drop PDF, DOCX, TXT, or MD here. Jarvis extracts readable context before synthesis.';
  const preview = document.getElementById('c-json-preview');
  if (preview) {
    preview.value = '';
    preview.style.borderColor = '';
  }
  setIntakeReviewStatus('Analyze a client brief to populate the structured profile. You can edit anything here before saving.', 'neutral');
}
...
async function synthesizeClient() {
  ...
  try {
      ...
      if(data.status === "success") {
          ...
          showNotification(
            "Brief Analyzed",
            "Jarvis successfully synthesized the client brief into a structured brand profile.",
            false,
...
function refreshOrchestratorIntroCard() {
    ...
    if(!intro) return;
    intro.innerHTML = `Type the release directly into the thread. Example: <strong style="color:var(--t1);">post @Clarke_Burgers . Single Burger now, and also post @Torque_District . Car Carousel tonight at 8:00 PM</strong>. I will break each clause into a release card, flag anything ambiguous, and wait for your confirmation before execution.`;
}
...
function openOrchestratorExecutionDetails(forceState = true) {
    ...
    const activePanel = document.getElementById('mission-control-run') || detail;
...
function renderMissionControlPlanCard() {
    ...
    host.innerHTML = `<div style="display:flex;justify-content:space-between;gap:10px;margin-bottom:10px;"><div><div style="font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--t4);font-family:'Space Mono';">Release Preview</div><div style="font-size:14px;color:var(--t2);margin-top:6px;">${escapeHtml(String(orchestratorMissionPlan.action || '').replace(/_/g, ' '))} - ${orchestratorMissionPlan.totals?.ready || 0}/${orchestratorMissionPlan.totals?.total || 0} ready</div></div><div style="font-size:11px;color:${orchestratorMissionPlan.can_run ? 'var(--green)' : 'var(--red)'};font-family:'Space Mono';text-transform:uppercase;">${orchestratorMissionPlan.can_run ? 'Ready' : 'Blocked'}</div></div><div style="font-size:12px;color:${orchestratorMissionPlan.can_run ? 'var(--green)' : 'var(--amber)'};line-height:1.65;margin-bottom:12px;">${orchestratorMissionPlan.can_run ? 'Preview complete. Jarvis can start this release sequentially now.' : 'Preview complete. Resolve the blocked items below before Jarvis can start the loaded release.'}</div>${warnings.length ? `<div style="margin-bottom:10px;padding:10px 12px;border-radius:12px;background:rgba(224,168,47,.08);border:1px solid rgba(224,168,47,.18);color:var(--amber);font-size:12px;line-height:1.6;">${warnings.map(escapeHtml).join('<br>')}</div>` : ''}<div style="display:flex;flex-direction:column;gap:8px;">${(orchestratorMissionPlan.items || []).map(item => `<div style="padding:10px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.06);background:${item.status === 'blocked' ? 'rgba(224,85,85,.08)' : 'rgba(255,255,255,.02)'};"><div style="font-size:13px;color:var(--t1);">${escapeHtml(String(item.summary || 'Draft'))}</div><div style="font-size:12px;color:var(--t4);margin-top:4px;">${escapeHtml(String(item.platform_label || 'No platforms'))}</div>${item.warning ? `<div style="font-size:12px;color:${item.status === 'blocked' ? 'var(--red)' : 'var(--amber)'};margin-top:6px;">${escapeHtml(item.warning)}</div>` : ''}</div>`).join('')}</div>`;
    renderJarvisBatchSummary();
}
...
function renderMissionControlRunTimeline() {
    ...
    if(!orchestratorMissionRun) {
        ...
        host.innerHTML = `<div style="font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--t4);font-family:'Space Mono';margin-bottom:10px;">Live Supervision</div><div style="font-size:14px;color:var(--t2);line-height:1.7;">No active release yet.</div><div style="font-size:12px;color:var(--t4);line-height:1.65;margin-top:8px;">Once you start a release, Jarvis will show each item here with its current phase, final outcome, and any approval handoff.</div>`;
        renderJarvisBatchSummary();
        return;
    }
    ...
    host.innerHTML = `<div style="display:flex;justify-content:space-between;gap:10px;margin-bottom:10px;"><div><div style="font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--t4);font-family:'Space Mono';">Live Supervision</div><div style="font-size:14px;color:var(--t2);margin-top:6px;">${escapeHtml(String(orchestratorMissionRun.status || 'queued').replace(/_/g, ' '))}</div></div><button type="button" data-orch-action="dismiss-run" class="orch-inline-action-btn is-quiet" style="padding:8px 10px;min-width:auto;">Dismiss</button></div><div style="display:flex;flex-direction:column;gap:8px;max-height:340px;overflow:auto;padding-right:4px;">${(orchestratorMissionRun.items || []).map((item, i) => { const status = String(item.status || '').toLowerCase(); const tone = status === 'failed' ? 'var(--red)' : status === 'partial_success' ? 'var(--amber)' : status === 'scheduled' ? 'var(--blue)' : status === 'approval_ready' || status === 'approval_sent_whatsapp' ? 'var(--purple)' : 'var(--green)'; return `<div style="padding:10px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.06);background:rgba(255,255,255,.02);"><div style="display:flex;justify-content:space-between;gap:10px;"><div><div style="font-size:13px;color:var(--t1);">${i + 1}. ${escapeHtml(String(item.summary || 'Draft'))}</div><div style="font-size:12px;color:var(--t4);margin-top:4px;">${escapeHtml(String(item.phase || 'Queued'))}</div>${item.message ? `<div style="font-size:12px;color:var(--t3);margin-top:6px;line-height:1.6;">${escapeHtml(item.message)}</div>` : ''}</div><div style="font-size:11px;color:${tone};font-family:'Space Mono';text-transform:uppercase;">${escapeHtml(String(item.status || 'queued').replace(/_/g, ' '))}</div></div></div>`; }).join('')}</div>`;
    renderJarvisBatchSummary();
}
...
async function previewMissionControlPlan(options = {}) {
    ...
    try {
        ...
        renderMissionControlPlanCard();
        renderJarvisBatchSummary();
        if(options.forceOpenDetails || !orchestratorMissionPlan?.can_run) openOrchestratorExecutionDetails(true);
...
async function hydrateMissionControlRun(silent = false) {
    ...
    try {
        ...
        const data = await res.json();
        if(!res.ok || data.status !== 'success') throw new Error(data.detail || data.reason || 'Run not found.');
    ...
    } catch(err) { if(!silent) toast(err?.message || 'Failed to load the run state.', 'error'); }
}
...
function renderVaultBundles() {
    ...
    bundleKeys.forEach(bName => {
        ...
        let imgsHtml = items.map(item => {
            ...
            if(item.kind === 'video') {
                if(posterUrl) {
                    return `<img src="${posterUrl}" style="width:64px;height:64px;object-fit:cover;border-radius:10px;${isBad ? 'border:2px solid var(--red);' : ''}" title="${isBad ? fObj.warning : ''}" />`;
                }
                return `<div style="width:64px;height:64px;display:flex;align-items:center;justify-content:center;border-radius:10px;background:linear-gradient(180deg, rgba(15,18,28,.92), rgba(5,7,12,.98));color:var(--t2);font-size:9px;font-family:'Space Mono';${isBad ? 'border:2px solid var(--red);' : ''}" title="${isBad ? fObj.warning : ''}">VIDEO</div>`;
            }
