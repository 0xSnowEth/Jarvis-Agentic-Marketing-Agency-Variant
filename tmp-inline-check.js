...
const pendingVaultUploadsByClient = {};
const API_BASE = (() => {
    try {
...
function revokePendingVaultPreview(record) {
    const previewUrl = String(record?.thumb_url || '').trim();
    if(previewUrl && previewUrl.startsWith('blob:')) {
        try { URL.revokeObjectURL(previewUrl); } catch(e) {}
    }
}

function getPendingVaultUploads(clientId = currentVaultClient) {
    const key = String(clientId || '').trim();
    return Array.isArray(pendingVaultUploadsByClient[key]) ? pendingVaultUploadsByClient[key] : [];
}

function buildPendingVaultAssetRecord(file) {
    const type = String(file?.type || '').toLowerCase();
    const previewUrl = type.startsWith('image/') ? URL.createObjectURL(file) : '';
    return {
        filename: String(file?.name || 'uploading-asset').trim(),
        kind: type.startsWith('video/') ? 'video' : 'image',
        is_video: type.startsWith('video/'),
        is_valid_ig: true,
        warning: '',
        can_repair_meta: false,
        has_poster: false,
        width: 0,
        height: 0,
        mime_type: type,
        size_bytes: Number(file?.size || 0) || 0,
        version_token: `pending-${Date.now()}`,
        ...
        _pending_name: String(file?.name || '').trim(),
    };
}

function addPendingVaultUploads(clientId, files) {
    const safeClientId = String(clientId || '').trim();
    const incomingFiles = Array.from(files || []);
    if(!safeClientId || !incomingFiles.length) return;
    const nextRecords = incomingFiles.map(buildPendingVaultAssetRecord).filter(record => record.filename);
    if(!nextRecords.length) return;
    const existing = getPendingVaultUploads(safeClientId);
    const merged = new Map(existing.map(record => [record.filename, record]));
    nextRecords.forEach(record => merged.set(record.filename, record));
    pendingVaultUploadsByClient[safeClientId] = Array.from(merged.values());
    if(currentVaultClient === safeClientId) {
        renderCurrentVaultState();
    }
}

function clearPendingVaultUploads(clientId, filenames = null) {
    const safeClientId = String(clientId || '').trim();
    if(!safeClientId || !pendingVaultUploadsByClient[safeClientId]) return;
    const filenameSet = Array.isArray(filenames) && filenames.length
        ? new Set(filenames.map(name => String(name || '').trim()).filter(Boolean))
        : null;
    const nextRecords = [];
    pendingVaultUploadsByClient[safeClientId].forEach(record => {
        const shouldRemove = !filenameSet || filenameSet.has(String(record?._pending_name || record?.filename || '').trim());
        if(shouldRemove) {
            revokePendingVaultPreview(record);
        } else {
            nextRecords.push(record);
        }
    });
    if(nextRecords.length) {
        pendingVaultUploadsByClient[safeClientId] = nextRecords;
    } else {
        delete pendingVaultUploadsByClient[safeClientId];
    }
    if(currentVaultClient === safeClientId) {
        renderCurrentVaultState();
    }
...
function getAvailableVaultFiles() {
    ...
    currentVaultFiles.forEach(item => {
        const filename = typeof item === 'string' ? item : item?.filename;
        if(filename) merged.set(filename, item);
    });
    getPendingVaultUploads(currentVaultClient).forEach(item => {
        const filename = String(item?.filename || '').trim();
        if(filename && !merged.has(filename)) merged.set(filename, item);
    });
...
function mergeUploadedAssetsIntoVault(clientId, uploadedAssets) {
    ...
    clientVaultCounts[clientId] = Math.max(Number(clientVaultCounts[clientId] || 0), Array.from(cachedMerged.values()).length);
...
function removeVaultDraftBundle(clientId, draftName) {
    const safeClientId = String(clientId || '').trim();
    const safeDraftName = String(draftName || '').trim();
    if(!safeClientId || !safeDraftName) return;
    if(currentVaultClient === safeClientId && currentVaultBundles[safeDraftName]) {
        delete currentVaultBundles[safeDraftName];
        renderCurrentVaultState();
    }
    const cached = vaultDraftsCache[safeClientId];
    if(cached && cached.bundles && cached.bundles[safeDraftName]) {
        delete cached.bundles[safeDraftName];
        vaultDraftsCache[safeClientId] = {
            ...cached,
...
function isWhatsAppOnlyApproval(job, fallbackRouting = '') {
  const mode = String(job?.approval_routing || fallbackRouting || '').trim().toLowerCase();
  return mode === 'whatsapp_only';
}
...
function openApprovalReviewModal(approvalId) {
  ...
  const whatsappOnly = isWhatsAppOnlyApproval(job);
  badges.innerHTML = [
    `<span class="badge b-pu">${escapeHtml(summarizeApprovalAssets(job))}</span>`,
      `<span class="badge ${job.whatsapp_sent ? 'b-on' : 'b-am'}">${whatsappOnly ? 'Owner WhatsApp Control' : (job.whatsapp_sent ? 'WhatsApp Sent' : 'Desktop Review')}</span>`,
  ...
  document.getElementById('approval-review-brief').innerHTML = [
    ...
    ...(whatsappOnly ? [renderApprovalBriefLine('Approval Lane', 'This release is locked to the WhatsApp owner lane. The owner must approve, move, or decline it from WhatsApp.')] : []),
  ].join('');

  const approveBtn = document.getElementById('approval-review-approve');
  const moveBtn = document.getElementById('approval-review-move');
  const refineBtn = document.getElementById('approval-review-refine');
  const whatsappBtn = document.getElementById('approval-review-whatsapp');
  approveBtn.style.display = whatsappOnly ? 'none' : '';
  moveBtn.style.display = whatsappOnly ? 'none' : '';
  refineBtn.style.display = whatsappOnly ? 'none' : '';
  whatsappBtn.textContent = job.whatsapp_sent ? 'Resend WhatsApp' : 'Send to WhatsApp';
  approveBtn.onclick = () => approveFromCenter(approvalId, { skipConfirm: false, closeModal: true });
  moveBtn.onclick = () => {
    closeApprovalReviewModal();
    openApprovalMoveModal(approvalId, String(job.client || '').replace(/[_-]/g, ' '));
  };
  refineBtn.onclick = () => rejectFromCenter(approvalId, { skipConfirm: false, closeModal: true });
...
async function renderApprovalCenter() {
  ...
  try {
    ...
    const markup = approvals.map(job => {
      ...
      const whatsappOnly = isWhatsAppOnlyApproval(job, routingMode);
      ...
      const whatsappBadge = job.whatsapp_sent
        ? `<span class="badge b-on">${whatsappOnly ? 'Owner WhatsApp Control' : 'Sent to WhatsApp'}</span>`
        : `<span class="badge b-pu">${whatsappOnly ? 'Waiting on WhatsApp' : 'Desktop Review'}</span>`;
      const sendButton = whatsappOnly
        ...
        : `<button class="hq-ghost-btn" onclick="sendApprovalToWhatsApp('${approvalId}')">${job.whatsapp_sent ? 'Resend WhatsApp' : 'Send to WhatsApp'}</button>`;
      const desktopActionButtons = whatsappOnly
...
function renderOrchestratorApprovalCard(reply, action) {
  ...
  const whatsappOnly = isWhatsAppOnlyApproval(job, action?.approval_routing);
  const actionsMarkup = whatsappOnly
    ? `
...
async function renderDashboardSummary() {
  ...
  window.__seenPublishFailureAlerts = window.__seenPublishFailureAlerts || new Set();
  ...
  try {
    ...
    const failedRuns = Array.isArray(s.recent_failed_publish_runs) ? s.recent_failed_publish_runs : [];
    ...
    failedRuns.slice(0, 4).forEach(run => {
      const failureBits = Array.isArray(run.platform_errors)
        ? run.platform_errors.map(item => {
            const step = item.step ? ` (${item.step})` : '';
            return `${String(item.platform || '').replace(/^\w/, c => c.toUpperCase())}: ${item.message}${step}`;
          })
        : [];
      blockedEntries.push(renderWorkItem(
        `${String(run.client_id || 'Unknown').replace(/_/g, ' ')} · ${run.topic || 'Scheduled publish'}`,
        escapeHtml(failureBits[0] || run.summary || 'A scheduled publish failed and needs operator attention.'),
        {
          kicker: 'Publish failed',
          meta: renderWorkBadges([
            renderWorkBadge('Failure', 'danger'),
            renderWorkBadge(run.failure_step || 'publish', 'warn'),
          ]),
          actionLabel: 'Open Calendar',
          actionOnclick: "nav('schedule', document.getElementById('nav-schedule'))",
        }
      ));
      const alertKey = String(run.run_id || `${run.client_id}:${run.topic}:${run.created_at || ''}`);
      if(alertKey && !window.__seenPublishFailureAlerts.has(alertKey)) {
        window.__seenPublishFailureAlerts.add(alertKey);
        const toastMessage = failureBits[0] || run.summary || 'A scheduled publish failed and needs attention.';
        showNotification('Publish Failed', escapeHtml(toastMessage), true, { position: 'bottom-right', duration: 6500 });
      }
    });
...
function showNotification(title, message, isError = false, options = {}) {
  ...
  if(position === 'bottom-right') {
    hud.style.right = '28px';
    hud.style.bottom = '26px';
    hud.style.transform = 'translate(0, 18px) scale(0.985)';
  } else if(position === 'bottom-center') {
    hud.style.left = '50%';
    hud.style.bottom = '26px';
    hud.style.transform = 'translate(-50%, 18px) scale(0.985)';
  } else {
    hud.style.top = '8%';
    hud.style.left = '50%';
    hud.style.transform = 'translate(-50%, -18px) scale(0.985)';
  ...
  hud.style.transition = 'opacity 0.32s cubic-bezier(0.22, 1, 0.36, 1), transform 0.42s cubic-bezier(0.22, 1, 0.36, 1)';
  hud.style.willChange = 'opacity, transform';
  ...
  setTimeout(() => {
      hud.style.opacity = '0';
      hud.style.transform = position === 'bottom-right'
        ? 'translate(0, 12px) scale(0.99)'
        : position === 'bottom-center'
        ? 'translate(-50%, 12px) scale(0.99)'
        : 'translate(-50%, 12px) scale(0.99)';
      setTimeout(() => hud.remove(), 400);
  }, duration);
}
...
async function saveClientProfile() {
  ...
  try {
      ...
      showNotification(
        ...
        false,
        { accent: INTAKE_CRYSTAL, rgb: INTAKE_CRYSTAL_RGB, messageColor: INTAKE_CRYSTAL_SOFT, duration: 5200, position: 'bottom-right' }
      );
      ...
      returnViewportToTop();
      resetClientIntakeForm();
      refreshClientWorkspaceViews().catch(() => null);
    ...
    try {
        ...
        if(data.status === "success") {
            ...
            if(errMatch) {
                ...
                if(await tryRenderPublishSuccessCard(msgNode, reply, data.draft_refs || outgoingDraftRefs)) {
...
async function resolvePublishSuccessMedia(refs = []) {
    ...
    if(!clientId) return null;
    try {
        const directItems = Array.isArray(first?.items) ? first.items : [];
        const directFirstItem = directItems[0] || null;
        const directFilename = String(directFirstItem?.filename || '').trim();
        const directKind = String(directFirstItem?.kind || '').trim().toLowerCase();
        if(directFilename) {
            const directThumb = String(
                (directKind === 'video' ? directFirstItem?.poster_thumb_url : directFirstItem?.thumb_url)
                || directFirstItem?.preview_url
                || ''
            ).trim();
            const directFull = String(
                (directKind === 'video' ? directFirstItem?.poster_full_url : directFirstItem?.full_url)
                || directFirstItem?.url
                || ''
            ).trim();
            if(directThumb || directFull) {
                return {
                    url: directThumb || directFull,
                    label: directFilename,
                    isVideo: directKind === 'video',
                    clientId,
                };
            }
            const cachedAssets = (vaultAssetsCache[clientId] && Array.isArray(vaultAssetsCache[clientId].files))
                ...
                : [];
            let directAsset = cachedAssets.find(item => String(item?.filename || '').trim() === directFilename) || null;
            if(!directAsset && currentVaultClient === clientId) {
                directAsset = (Array.isArray(currentVaultFiles) ? currentVaultFiles : []).find(item => String((typeof item === 'string' ? item : item?.filename) || '').trim() === directFilename) || null;
            }
            if(!directAsset) {
                try {
                    const assets = await fetchVaultAssets(clientId);
                    directAsset = (Array.isArray(assets?.files) ? assets.files : []).find(item => String(item?.filename || '').trim() === directFilename) || null;
                } catch(_) {}
            }
            if(directAsset) {
                const isVideo = String(directAsset?.kind || directKind || '').toLowerCase() === 'video';
                return {
                    url: isVideo ? getVaultAssetPosterUrl(clientId, directAsset) : getVaultAssetPreviewUrl(clientId, directAsset),
                    label: directFilename,
                    isVideo,
                    clientId,
                };
            }
            return buildDirectVaultMediaFallback(clientId, directFirstItem);
        }
...
function renderPublishSuccessCard(payload, media = null) {
    ...
    `).join('');
    const mediaFallbackMarkup = `<div class="publish-success-media-fallback">Live release<br/>confirmed</div>`;
    const mediaMarkup = media?.url
        ? `<img src="${escapeHtml(media.url)}" alt="${escapeHtml(media.label || clientLabel)}" loading="eager" decoding="async" width="140" height="140" onerror="this.onerror=null; this.insertAdjacentHTML('afterend', \`${mediaFallbackMarkup}\`); this.remove();" />`
        : mediaFallbackMarkup;
    return `
...
async function tryRenderPublishSuccessCard(node, reply, refs = []) {
    ...
    node.style.cssText = '';
    node.innerHTML = renderPublishSuccessCard(payload, null);
    resolvePublishSuccessMedia(refs)
        .then(media => {
            if(node.isConnected && media) node.innerHTML = renderPublishSuccessCard(payload, media);
        })
        .catch(() => null);
    return true;
}
...
async function handleVaultUpload(source) {
    ...
    addPendingVaultUploads(clientId, allFiles);
    showNotification('Adding Assets', `${allFiles.length} asset${allFiles.length === 1 ? '' : 's'} are appearing in the vault now and will finish syncing in the background.`, false, { state: 'pending', duration: 1600, position: 'bottom-right' });
    ...
    try {
        ...
        for (const batch of orderedBatches) {
            ...
            if (clientId === currentVaultClient) {
                ...
                clearPendingVaultUploads(clientId, batch.files.map(file => file.name));
                nameEl.innerHTML = `<span style="color:var(--amber)">${batch.label === 'image' ? 'Images' : 'Assets'} ready.</span>`;
        ...
        clearPendingVaultUploads(clientId);
...
async function openVaultModal(clientId) {
    ...
    if(cachedAssets) renderVaultGrid(getAvailableVaultFiles());
    ...
    if(!cachedAssets) loadVaultAssetsData(true).catch(() => null);
    if(!cachedDrafts) loadVaultDraftsData(true).catch(() => null);
...
function renderVaultGrid(files) {
    ...
    files.forEach((fObj, index) => {
        ...
        const pendingBadge = fObj && typeof fObj === 'object' && fObj._pending_upload
            ? `<div class="vault-media-kind-badge" style="left:8px; right:auto; background:rgba(244,211,138,0.16); color:var(--amber); border-color:rgba(244,211,138,0.28);">Syncing</div>`
            : '';
...
async function saveDraftRename(oldName) {
    ...
    const previousDraft = currentVaultBundles[oldName] ? JSON.parse(JSON.stringify(currentVaultBundles[oldName])) : null;
    setButtonBusy(triggerButton, 'Renaming...');
    input.disabled = true;
    if(previousDraft) {
        removeVaultDraftBundle(currentVaultClient, oldName);
        upsertVaultDraftBundle(currentVaultClient, newName, previousDraft);
    }
    editingDraftName = null;
    renderVaultBundles();
    showNotification('Renaming Draft', `${oldName} is becoming ${newName} in the background.`, false, { state: 'pending', duration: 1200, position: 'bottom-right' });
    restoreButtonBusy(triggerButton);
    if(input) input.disabled = false;
    fetch(buildApiUrl(`/api/vault/${encodeURIComponent(currentVaultClient)}/bundles/${encodeURIComponent(oldName)}/rename`), {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        ...
        if(previousDraft) {
            removeVaultDraftBundle(currentVaultClient, newName);
            upsertVaultDraftBundle(currentVaultClient, oldName, previousDraft);
        }
...
async function createBundleFromSelection(button = null) {
    ...
    const optimisticDraft = {
        ...
        items: filesArray.map(name => {
            const record = currentVaultFiles.find(item => (typeof item === 'string' ? item : item.filename) === name);
            return {
                filename: name,
                kind: typeof record === 'string' ? 'image' : (record?.kind || 'image'),
            };
        }),
        caption_mode: 'ai',
    ...
    selectedVaultImages.clear();
    upsertVaultDraftBundle(currentVaultClient, bName, optimisticDraft);
    updateSelectionFooter();
    switchVaultTab('bundles');
    showNotification('Saving Draft', `${bName} is appearing in Creative Drafts now and will finish syncing in the background.`, false, { state: 'pending', duration: 1200, position: 'bottom-right' });
    restoreButtonBusy(button);
    fetch(buildApiUrl(`/api/vault/${encodeURIComponent(currentVaultClient)}/bundles`), {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        ...
        removeVaultDraftBundle(currentVaultClient, bName);
...
async function deleteBundle(bName) {
    showConfirm(`Remove draft ${bName}? The source assets stay in Available Assets.`, async () => {
        try {
